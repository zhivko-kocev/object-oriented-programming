...
Да се развие класа Oglasnik во која се чуваат податоци за име на огласникот
(текстуална низа од максимум 20 знаци), низа од огласи (динамички резервирана низа од објекти од класата Oglas)
и број на огласи во низата (цел број) (5 поени)
...
public:
    ...
    Oglasnik(const Oglasnik &o)
    {
        ...
        this->oglasi = new Oglas[o.brOglasi];
    ...
    Oglasnik &operator+=(const Oglas &o)
    {
        ...
        else
        {
            if (!brOglasi)
            {
                this->oglasi = new Oglas[1];
                this->oglasi[brOglasi++] = o;
            }
            else
            {
    ...
    void oglasiOdKategorija(const char *k)
    {
        for (int i = 0; i < brOglasi; ++i)
        {
            if (!strcmp(k, oglasi[i].getCategory()))
            {
                cout << oglasi[i] << endl;
            }
        }
    ...
    void najniskaCena()
    {
        ...
        for (int i = 0; i < brOglasi; ++i)
        {
            if (oglasi[i].getPrice() < min.getPrice())
            {
                min = oglasi[i];
            }
        }
